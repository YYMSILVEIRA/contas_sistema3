...
if ($id) {
    ...
    $sql = "SELECT cr.id, cr.descricao, cr.valor, cr.data, cr.cliente, cr.id_empresa, cr.id_situacao, 
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['descricao'])) {
    ...
    $sql = "UPDATE contas_receber 
            SET descricao = ?, valor = ?, data = ?, cliente = ?, id_situacao = ? 
            WHERE id = ? AND id_empresa = (SELECT id_empresa FROM usuarios WHERE id = ?)";
    $stmt = $pdo->prepare($sql);
    $stmt->execute([$descricao, $valor, $data, $cliente, $situacao, $id, $user_id]);
...
<head>
    ...
    <title>Editar Registro Manualmente</title>
    <!-- Bootstrap CSS -->
