...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $pdo->prepare('INSERT INTO contas_receber (descricao, valor, data, cliente, id_empresa) VALUES (?, ?, ?, ?, (SELECT id_empresa FROM usuarios WHERE id = ?))');
    $stmt->execute([$descricao, $valor, $data, $cliente, $_SESSION['user_id']]);
}

// Consulta para buscar as contas a receber com a situação
$sql = "
    SELECT cr.id, cr.descricao, cr.valor, DATE_FORMAT(cr.data, '%d/%m/%Y') as data, cr.cliente, s.descricao as situacao 
    FROM contas_receber cr
    LEFT JOIN situacoes s ON cr.id_situacao = s.id
    WHERE cr.id_empresa = (SELECT id_empresa FROM usuarios WHERE id = :user_id) 
    AND cr.id_situacao IN (8,7,5)
";

// Adiciona condições de pesquisa se houver um termo
if (!empty($termoPesquisa)) {
    $sql .= " AND (cr.descricao LIKE :termo1 OR cr.valor LIKE :termo2 OR cr.data LIKE :termo3 OR cr.cliente LIKE :termo4 OR s.descricao LIKE :termo5)";
}
...
if (!empty($termoPesquisa)) {
    $sqlTotal .= " AND (cr.descricao LIKE :termo1 OR cr.valor LIKE :termo2 OR cr.data LIKE :termo3 OR cr.cliente LIKE :termo4 OR s.descricao LIKE :termo5)";
}
...
<body>
    <div class="container">
        <!-- Cabeçalho -->
        <?= Cabecalho("Contas a Receber", "Dashboard", "Adicionar Conta a Receber", "Relatórios", "dashboard.php", "adicionar_contas_receber.php", "relatorios.php"); ?>

        <!-- Título -->
        <h3 class="mt-4">Lista de Contas a Receber</h3>
        ...
        <form method="get" action="" class="mb-4">
            <div class="input-group">
                ...
                <?php if (!empty($termoPesquisa)): ?>
                    <a href="contas_receber.php" class="btn btn-secondary">Limpar Pesquisa</a>
                <?php endif; ?>
            </div>
        </form>
        ...
        <table class="table table-striped table-bordered">
            <thead>
                <tr>
                    ...
                    <th class="sortable" onclick="ordenarPor('data')">Receber Até <?= setaOrdenacao('data', $ordenarPor, $ordenacao) ?></th>
                    <th class="sortable" onclick="ordenarPor('cliente')">Cliente <?= setaOrdenacao('cliente', $ordenarPor, $ordenacao) ?></th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <a href="<?= 'editar_conta_receber.php?id='.$conta['id'] ?>" class="btn-editar">Editar</a>
                        <a href="<?= 'excluir_conta_receber.php?id='.$conta['id'] ?>" class="btn-excluir" onclick="return confirm('Tem certeza que deseja excluir esta conta?')">Excluir</a>
                    </td>
                </tr>
                <?php endforeach; ?>
            </tbody>
    ...
    <script>
        ...
        function ordenarPor(coluna) {
            ...
            window.location.href = `contas_receber.php?${urlParams.toString()}`;
        }
    </script>
</body>
</html>
